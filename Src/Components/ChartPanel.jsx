...
export default function ChartPanel({ pitcher }) {
  ...
  useEffect(() => {
    ...
    channelRef.current = supabase
      .channel('pitches-' + activeSessionId)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'pitches', filter: `session_id=eq.${activeSessionId}` },
        payload => setPitches(prev => [...prev, payload.new]))
      .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'pitches', filter: `session_id=eq.${activeSessionId}` },
        payload => setPitches(prev => prev.filter(p => p.id !== payload.old.id)))
      .subscribe()
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px 16px', overflowY: 'auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexWrap: 'wrap', justifyContent: 'center', marginBottom: '16px', maxWidth: '860px' }}>
        <button onClick={() => { setActiveSessionId(null); setFilter('All') }}
          style={{ padding: '7px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold', letterSpacing: '1px', border: 'none',
            background: isTotal ? 'linear-gradient(135deg,#6366f1,#8b5cf6)' : 'rgba(99,102,241,0.12)',
            color: isTotal ? 'white' : '#94a3b8',
            boxShadow: isTotal ? '0 0 16px rgba(99,102,241,0.4)' : 'none' }}>
          ⬡ TOTAL
        </button>
        ...
        {sessions.map(s => (
          <div key={s.id} style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
            {editingSession === s.id ? (
              ...
              <button
                onClick={() => { setActiveSessionId(s.id); setFilter('All') }}
                ...
                style={{ padding: '7px 12px', paddingRight: '26px', borderRadius: '8px', cursor: 'pointer', fontSize: '11px', border: 'none',
                  ...
                  borderBottom: activeSessionId === s.id ? '2px solid #e2e8f0' : '2px solid transparent' }}>
                {s.name}
              </button>
            )}
      ...
      {isTotal && (
        <div style={{ fontSize: '10px', color: '#6366f1', marginBottom: '10px', letterSpacing: '1px' }}>
          ⬡ AGGREGATE — {sessions.length} SESSION{sessions.length !== 1 ? 'S' : ''} · READ ONLY
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'flex-start' }}>
        ...
        <div style={{ width: '160px' }}>
          <Sec label="PITCH TYPE">
            {PITCH_TYPES.map(pt => (
              <CBtn key={pt} active={selType === pt} onClick={() => setSelType(pt)} color="#3b82f6" disabled={isTotal}>{pt}</CBtn>
            ))}
          </Sec>
          <Sec label="QUALITY">
            {Object.entries(CATS).map(([key, val]) => (
              <CBtn key={key} active={selCat === key} onClick={() => setSelCat(key)} color={`rgb(${val.rgb.join(',')})`} dot disabled={isTotal}>
                {val.label}
              </CBtn>
            ))}
          </Sec>
          <Sec label="FILTER">
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
              {['All', ...PITCH_TYPES].map(ft => (
                <button key={ft} onClick={() => setFilter(ft)}
                  style={{ padding: '4px 8px', background: filter === ft ? '#6366f1' : 'rgba(255,255,255,0.05)', border: '1px solid ' + (filter === ft ? '#6366f1' : 'rgba(255,255,255,0.1)'), borderRadius: '4px', color: 'white', cursor: 'pointer', fontSize: '10px' }}>
                  {ft}
                </button>
              ))}
            </div>
          ...
          {!isTotal && (
            <div style={{ display: 'flex', gap: '6px', marginTop: '12px' }}>
              <button onClick={undo} style={xbtn('#374151')}>↩ Undo</button>
              <button onClick={clearSession} style={xbtn('#7f1d1d')}>✕ Clear</button>
            </div>
          )}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          {loading ? (
            <div style={{ width: CW, height: CH, background: '#1a2035', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#475569', fontSize: '11px', letterSpacing: '2px' }}>
              LOADING...
            </div>
          ) : (
          ...
          <div style={{ fontSize: '10px', color: '#475569', marginTop: '8px', letterSpacing: '1px' }}>
            {isTotal ? 'AGGREGATE · READ ONLY' : 'CLICK TO PLACE PITCH · DOUBLE-CLICK TAB TO RENAME'}
          </div>
        </div>
                ...
                return (
                  <div key={pt} style={{ padding: '10px', marginBottom: '8px', background: 'rgba(255,255,255,0.04)', borderRadius: '8px', borderLeft: '3px solid #3b82f6' }}>
                    ...
                    <div style={{ display: 'flex', height: '6px', borderRadius: '3px', overflow: 'hidden', marginBottom: '8px' }}>
                      <div style={{ width: `${ep}%`, background: 'rgb(34,197,94)', transition: 'width 0.3s' }} />
                      <div style={{ width: `${cp}%`, background: 'rgb(250,204,21)', transition: 'width 0.3s' }} />
                      <div style={{ width: `${np}%`, background: 'rgb(239,68,68)', transition: 'width 0.3s' }} />
                    </div>
...
function Sec({ label, children }) {
  return (
    <div style={{ marginBottom: '16px' }}>
      <div style={{ fontSize: '9px', letterSpacing: '2px', color: '#475569', marginBottom: '8px' }}>{label}</div>
      {children}
    </div>
  )
}
function CBtn({ active, onClick, color, children, dot, disabled }) {
  return (
    <button onClick={onClick} disabled={disabled}
      style={{ display: 'flex', alignItems: 'center', gap: '7px', width: '100%', marginBottom: '5px', padding: '7px 10px',
        background: active ? `${color}22` : 'rgba(255,255,255,0.04)',
        border: `1.5px solid ${active ? color : 'rgba(255,255,255,0.08)'}`,
        borderRadius: '6px', color: disabled ? '#374151' : 'white', cursor: disabled ? 'default' : 'pointer',
        fontSize: '11px', textAlign: 'left', fontWeight: active ? 'bold' : 'normal', opacity: disabled ? 0.4 : 1 }}>
