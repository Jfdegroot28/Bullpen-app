...
export default function ChartPanel({ pitcher, onUpdatePitcher }) {
  ...
  const [trendMetric, setTrendMetric] = useState('exec')
  ...
  useEffect(() => {
    ...
    setMode('log'); setBuildSteps([]); setBuildName(''); setRunScript(null); setRunIndex(0); setSelectedPitchId(null); setMovingPitch(false); setShowTrends(false)
    const load = async () => {
  ...
  useEffect(() => {
    if (!showTrends || !pitcher || sessions.length === 0) { setTrendRows([]); return }
    const load = async () => {
      const ids = sessions.map(s => s.id)
      const { data } = await supabase.from('pitches').select('*').in('session_id', ids)
      const byId = {}
      ;(data || []).forEach(p => { (byId[p.session_id] = byId[p.session_id] || []).push(p) })
      const rows = sessions.map(s => {
        let pts = byId[s.id] || []
        if (trendType !== 'All') pts = pts.filter(p => p.pitch_type === trendType)
        const n = pts.length
        const ex = pts.filter(p => p.quality === 'executed').length
        const execPct = n ? Math.round(ex / n * 100) : null
        const tp = pts.filter(p => p.target_x != null && p.target_y != null)
        const avgMiss = tp.length ? (tp.reduce((a,p)=>a+Math.hypot(p.x-p.target_x, p.y-p.target_y),0)/tp.length*INCHES_PER_PX) : null
        return { id: s.id, name: s.name, date: s.session_date, n, execPct, avgMiss }
      })
      setTrendRows(rows)
    }
    load()
  }, [showTrends, sessions, pitcher?.id, trendType])

  useEffect(() => {
    if (!pitcher || showTrends) return
  ...
  }, [displayPitches, filter, showDots, showTargets, showSpread, pendingTarget, buildSteps, selectedPitchId, mode, pitcher, activeSessionId, sessions, isTotal, showTrends])
  ...
  const addSession = async () => {
    ...
    setSessions(prev => [...prev, data]); setActiveSessionId(data.id); setShowTrends(false)
  }
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px 16px', overflowY: 'auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexWrap: 'wrap', justifyContent: 'center', marginBottom: '16px', maxWidth: '860px' }}>
        <button onClick={() => { setActiveSessionId(null); setFilter('All'); setShowTrends(false) }}
          style={{ padding: '7px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold', letterSpacing: '1px', border: 'none',
            background: (isTotal && !showTrends) ? 'linear-gradient(135deg,#f5a623,#e09000)' : 'rgba(245,166,35,0.12)',
            color: (isTotal && !showTrends) ? '#0f1a3d' : '#94a3b8',
            boxShadow: (isTotal && !showTrends) ? '0 0 16px rgba(245,166,35,0.4)' : 'none' }}>
          ⬡ TOTAL
        </button>
        <button onClick={() => setShowTrends(t => !t)}
          style={{ padding: '7px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold', letterSpacing: '1px', border: 'none',
            background: showTrends ? 'linear-gradient(135deg,#f5a623,#e09000)' : 'rgba(245,166,35,0.12)',
            color: showTrends ? '#0f1a3d' : '#94a3b8',
            boxShadow: showTrends ? '0 0 16px rgba(245,166,35,0.4)' : 'none' }}>
          📈 TRENDS
        </button>
        {sessions.map(s => (
          <div key={s.id} style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
            {editingSession === s.id ? (
              ...
              <button onClick={() => { setActiveSessionId(s.id); setFilter('All'); setShowTrends(false) }}
                ...
                style={{ padding: '7px 12px', paddingRight: '26px', borderRadius: '8px', cursor: 'pointer', fontSize: '11px', border: 'none',
                  background: (activeSessionId === s.id && !showTrends) ? 'rgba(255,255,255,0.12)' : 'rgba(255,255,255,0.05)',
                  color: (activeSessionId === s.id && !showTrends) ? 'white' : '#64748b',
                  borderBottom: (activeSessionId === s.id && !showTrends) ? '2px solid #f5a623' : '2px solid transparent' }}>
                {s.name}
              </button>
            )}
      ...
      {showTrends ? (
        <TrendsPanel rows={trendRows} metric={trendMetric} setMetric={setTrendMetric} type={trendType} setType={setTrendType} pitchTypes={pitchTypes} />
      ) : (
        <>
...
function TrendChart({ rows, metric }) {
  const W = 620, H = 280, padL = 44, padR = 16, padT = 20, padB = 56
  const pts = rows.map((r, i) => ({ ...r, i, v: metric === 'exec' ? r.execPct : r.avgMiss }))
  const valid = pts.filter(p => p.v != null)
  if (valid.length === 0) return <div style={{ color: '#64748b', fontSize: '12px', padding: '30px', textAlign: 'center' }}>No data yet for this selection.</div>
  const maxV = metric === 'exec' ? 100 : Math.max(8, Math.ceil(Math.max(...valid.map(p => p.v)) / 2) * 2)
  const plotW = W - padL - padR, plotH = H - padT - padB
  const xFor = i => padL + (pts.length === 1 ? plotW / 2 : (i / (pts.length - 1)) * plotW)
  const yFor = v => padT + plotH - (v / maxV) * plotH
  const ticks = metric === 'exec' ? [0, 25, 50, 75, 100] : [0, maxV / 4, maxV / 2, 3 * maxV / 4, maxV]
  const line = valid.map((p, k) => `${k === 0 ? 'M' : 'L'} ${xFor(p.i)} ${yFor(p.v)}`).join(' ')
  const color = metric === 'exec' ? '#22c55e' : '#f5a623'
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', maxWidth: W }}>
      {ticks.map((t, k) => (
        <g key={k}>
          <line x1={padL} y1={yFor(t)} x2={W - padR} y2={yFor(t)} stroke="rgba(255,255,255,0.08)" />
          <text x={padL - 8} y={yFor(t) + 3} fill="#64748b" fontSize="9" textAnchor="end" fontFamily="Courier New">{metric === 'exec' ? `${t}%` : `${Math.round(t)}"`}</text>
        </g>
      ))}
      <path d={line} fill="none" stroke={color} strokeWidth="2" />
      {valid.map((p, k) => <circle key={k} cx={xFor(p.i)} cy={yFor(p.v)} r="4" fill={color} stroke="#0f1a3d" strokeWidth="1.5" />)}
      {valid.map((p, k) => <text key={'v' + k} x={xFor(p.i)} y={yFor(p.v) - 9} fill={color} fontSize="9" textAnchor="middle" fontFamily="Courier New" fontWeight="bold">{metric === 'exec' ? `${p.v}%` : p.v.toFixed(1)}</text>)}
      {pts.map((p, k) => <text key={'n' + k} x={xFor(p.i)} y={H - padB + 18} fill="#94a3b8" fontSize="9" textAnchor="middle" fontFamily="Courier New" transform={`rotate(32 ${xFor(p.i)} ${H - padB + 18})`}>{p.name.length > 10 ? p.name.slice(0, 9) + '…' : p.name}</text>)}
    </svg>
  )
}

function TrendsPanel({ rows, metric, setMetric, type, setType, pitchTypes }) {
  return (
    <div style={{ width: '100%', maxWidth: '700px' }}>
      <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', justifyContent: 'center', marginBottom: '16px' }}>
        <div style={{ display: 'flex', gap: '4px' }}>
          {[['exec', 'Execution %'], ['miss', 'Avg miss']].map(([k, lbl]) => (
            <button key={k} onClick={() => setMetric(k)} style={{ padding: '6px 14px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold', background: metric === k ? '#f5a623' : 'rgba(255,255,255,0.06)', color: metric === k ? '#0f1a3d' : '#94a3b8' }}>{lbl}</button>
          ))}
        </div>
        <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
          {['All', ...pitchTypes].map(ft => (
            <button key={ft} onClick={() => setType(ft)} style={{ padding: '5px 10px', borderRadius: '5px', border: '1px solid ' + (type === ft ? '#f5a623' : 'rgba(255,255,255,0.1)'), cursor: 'pointer', fontSize: '10px', background: type === ft ? '#f5a623' : 'rgba(255,255,255,0.05)', color: type === ft ? '#0f1a3d' : 'white', fontWeight: type === ft ? 'bold' : 'normal' }}>{ft}</button>
          ))}
        </div>
      </div>
      <div style={{ background: 'rgba(0,0,0,0.25)', borderRadius: '12px', padding: '16px', border: '1px solid rgba(245,166,35,0.15)' }}>
        <TrendChart rows={rows} metric={metric} />
      </div>
      <div style={{ fontSize: '10px', color: '#64748b', textAlign: 'center', marginTop: '8px' }}>
        {metric === 'exec' ? 'Higher is better — % graded Executed per session.' : 'Lower is better — avg miss from target per session (needs target data).'}
      </div>
      <div style={{ marginTop: '16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0 12px 4px', fontSize: '8px', letterSpacing: '1px', color: '#475569' }}>
          <span>SESSION</span><span>EXEC% · MISS</span>
        </div>
        {rows.length === 0 ? (
          <div style={{ color: '#64748b', fontSize: '12px', textAlign: 'center', padding: '12px' }}>No sessions yet.</div>
        ) : rows.map(r => (
          <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', marginBottom: '5px', background: 'rgba(255,255,255,0.03)', borderRadius: '6px' }}>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              <span style={{ fontSize: '12px', color: 'white' }}>{r.name}</span>
              <span style={{ fontSize: '9px', color: '#64748b' }}>{r.date || ''} · {r.n} pitches</span>
            </div>
            <div style={{ display: 'flex', gap: '14px', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', color: '#22c55e', minWidth: '34px', textAlign: 'right' }}>{r.execPct == null ? '—' : `${r.execPct}%`}</span>
              <span style={{ fontSize: '12px', color: '#f5a623', minWidth: '34px', textAlign: 'right' }}>{r.avgMiss == null ? '—' : `${r.avgMiss.toFixed(1)}"`}</span>
            </div>
          </div>
        ))}
      </div>
